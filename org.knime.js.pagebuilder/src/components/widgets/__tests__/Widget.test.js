...
describe("Widget.vue", () => {
  ...
  it("publishes update to store", () => {
    ...
    expect(
      wrapper.vm.$store.state.pagebuilder.page.wizardPageContent.webNodes.id1
        .viewRepresentation.currentValue.testValue,
  ...
  it("updates objects inside arrays in the store", () => {
    let nodeId = "SINGLE";
    expect(
      wrapper.vm.$store.state.pagebuilder.page.wizardPageContent.webNodes.SINGLE
        .filters[0].test,
    ).toBe(0);
    wrapper.vm.publishUpdate({
      nodeId,
  ...
  it("registers a value getter with the store", () => {
    ...
    expect(
      typeof wrapper.vm.$store.state.pagebuilder.pageValueGetters[nodeId],
    ).toBe("function");
    expect(wrapper.vm.$store.state.pagebuilder.pageValueGetters[nodeId]).toBe(
      wrapper.vm.getValue,
    );
  });
  ...
  it("prevents value getter registration for incompatible types", () => {
    ...
    let newWrapper = shallowMount(Widget, {
      props: {
        nodeConfig: {
          ...
          viewRepresentation: {
            "@class":
              "org.knime.js.base.node.output.text.TextOutputRepresentation",
    ...
    expect(
      typeof newWrapper.vm.$store.state.pagebuilder.pageValueGetters[nodeId2],
    ).toBe("undefined");
  });
  ...
  it("registers a validator with the store", () => {
    ...
    expect(wrapper.vm.$store.state.pagebuilder.pageValidators[nodeId]).toBe(
      wrapper.vm.validate,
    );
  });
  ...
  it("updates values for reactive widgets without valueGetter (no onChange)", async () => {
    ...
    let updateSpy = vi
      .spyOn(localWrapper.vm, "updateWebNode")
      .mockResolvedValueOnce(true);
    await localWrapper.vm.$refs.widget.$emit("updateWidget", {
      type: "any",
      value: "some",
    });
  ...
  it("triggers re-execution if widget is reactive", async () => {
    ...
    let triggerSpy = vi
      .spyOn(localWrapper.vm, "triggerReExecution")
      .mockReturnValueOnce(true);
    await localWrapper.vm.$refs.widget.$emit("updateWidget", {
      type: "any",
      value: "some",
