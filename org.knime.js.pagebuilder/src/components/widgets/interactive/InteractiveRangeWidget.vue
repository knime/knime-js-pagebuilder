...
export default {
  ...
  computed: {
    ...
    valueMinimum() {
      ...
      return min === "-Infinity"
        ? false
        : parseFloat(min.toFixed(MAX_DECIMAL_PRECISION));
    },
    valueMaximum() {
      /* Currently vue-slider-component only supports limited precision. The backend is using
            BigDecimals, so it's possible to get a number with an very large precision. A feature
      ...
      return max === "Infinity"
        ? false
        : parseFloat(max.toFixed(MAX_DECIMAL_PRECISION));
    },
    isRangeSlider() {
    ...
    value() {
      if (!this.isRangeSlider) {
        return {
          [DATA_TYPE]:
            this.valueMinimum === false ? this.valueMaximum : this.valueMinimum,
        };
      }
  ...
  methods: {
    getValue() {
      ...
      value.filter.columns[0].maximum =
        typeof this.valueMaximum === "number" ? this.valueMaximum : "Infinity";
      value.filter.columns[0].minimum =
        typeof this.valueMinimum === "number" ? this.valueMinimum : "-Infinity";
      return value;
    },
    onChange(value) {
      let update = {};
      if (this.isRangeSlider) {
        ...
        let key =
          this.valueMinimum === false ? [UPDATE_KEY_MAX] : [UPDATE_KEY_MIN];
