...
describe("FileDownloadWidget.vue", () => {
  ...
  beforeEach(() => {
    ...
    props = {
      nodeConfig: {
        ...
        viewRepresentation: {
          "@class":
            "org.knime.js.base.node.output.filedownload.FileDownloadRepresentation",
          linkTitle: "SVG",
        ...
        viewValue: {
          "@class":
            "org.knime.js.base.node.output.filedownload.FileDownloadValue",
        },
        initMethodName: "init",
  ...
  it("shows error if no downloadResourceLink api is available", () => {
    ...
    expect(validate.errorMessage).toBe(
  ...
  it("displays a file link when run locally", () => {
    ...
    expect(fl.props("href")).toBe(
      `file://${props.nodeConfig.viewRepresentation.path}`,
    );
    delete window.KnimePageLoader;
  });

  it("uses a url when one is set in the representation", () => {
    downloadResourceLinkMock.mockReturnValue(null);
    window.KnimePageLoader = {};
    ...
    let fl = wrapper.findComponent(FileLink);
    expect(fl.exists()).toBeTruthy();
    expect(fl.props("href")).toBe("http://some.url");
    delete window.KnimePageLoader;
  });
});
