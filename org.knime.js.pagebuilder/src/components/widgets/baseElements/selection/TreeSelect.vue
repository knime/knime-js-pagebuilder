...
export default {
  ...
  methods: {
      ...
      if (this.multiple) {
        if (e.shiftKey) {
          if (oNode.$parent === this.lastClickedNode?.$parent) {
            ...
            let firstIndex = currentLevel.findIndex(
              (v) => v === this.lastClickedNode,
            );
    ...
    nextNode(startNode, delta) {
      let siblings = this.filterForTreeSelectItems(
        startNode.$parent.$refs.children,
      );
    ...
    filterForTreeSelectItems(children) {
      return (
        children?.filter((x) => x.$options.name === "TreeSelectItem") || []
      );
    },
    moveKeyBoardFocus(delta) {
      ...
      let down = !up;
      if (this.currentKeyboardNavNode === null) {
        const directTreeItems = this.filterForTreeSelectItems(
          this.$refs.children,
        );
      ...
      if (
        this.currentKeyboardNavNode.model.children.length > 0 &&
        this.currentKeyboardNavNode.model.opened &&
        down
      ) {
        // use first item of children (we go down)
        nextKeyboardNavNode = this.filterForTreeSelectItems(
          this.currentKeyboardNavNode.$refs.children,
        )[0];
      } else {
      ...
      if (childArrayOutOfBounds) {
        ...
        if (up) {
          ...
          // we go down
          nextKeyboardNavNode = this.nextNode(
            this.currentKeyboardNavNode.$parent,
            delta,
          );
      ...
      if (nextKeyboardNavNode.model.opened && up && !childArrayOutOfBounds) {
        ...
        const nextTreeItems = this.filterForTreeSelectItems(
          nextKeyboardNavNode.$refs.children,
        );
    ...
    onEnterKey(e) {
      ...
      if (
        this.currentKeyboardNavNode.model.disabled ||
        this.currentKeyboardNavNode.$data.isKeyNav === false
      ) {
        return;
      }
      if (this.multiple && (e.ctrlKey || e.metaKey)) {
        this.currentKeyboardNavNode.model.selected =
          !this.currentKeyboardNavNode.model.selected;
      } else {
      ...
      this.$emit(
        ...
        this.currentKeyboardNavNode.model,
        e,
      );
    },
    scrollToCurrent() {
...
<template>
  <div :class="['tree-wrapper', { invalid: !isValid }]">
    <div ref="treeDiv" class="tree" role="tree">
      <ul
