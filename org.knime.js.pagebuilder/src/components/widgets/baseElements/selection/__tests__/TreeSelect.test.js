...
describe("TreeSelect.vue", () => {
  ...
  it("renders custom icon", () => {
    ...
    expect(
      wrapper
        .findComponent(TreeSelectItem)
        .findComponent(customIcon)
        .isVisible(),
    ).toBeTruthy();
    expect(
      wrapper
        .findComponent(TreeSelectItem)
        .findComponent(customSelectedIcon)
        .exists(),
    ).toBeFalsy();
  });
  ...
  it("renders custom selected icon", () => {
    ...
    const wrapper = mount(TreeSelect, { props });
    expect(
      wrapper.findComponent(TreeSelectItem).findComponent(customIcon).exists(),
    ).toBeFalsy();
    expect(
      wrapper
        .findComponent(TreeSelectItem)
  ...
  describe("mouse interaction", () => {
    ...
    it("removes keyboard navigation state on hover", () => {
      ...
      expect(
        wrapper.findAllComponents(TreeSelectItem).at(0).vm.$data.isHover,
      ).toBe(true);
    });
  });
  ...
  describe("keyboard interaction", () => {
    ...
    it("navigates into open tree using up key", () => {
      ...
      expect(wrapper.vm.currentKeyboardNavNode.model.value).toBe("child2");
      expect(
        wrapper.findAllComponents(TreeSelectItem).at(4).vm.$data.isHover,
      ).toBe(false);
    });
  ...
  it("scrolls to element correctly", () => {
    props.data = [];
    const wrapper = shallowMount(TreeSelect, { props });
