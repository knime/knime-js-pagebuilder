...
export default {
  ...
  computed: {
    ...
    min() {
      ...
      return parseFloat(
        this.sliderSettings.range.min[0].toFixed(MAX_DECIMAL_PRECISION),
      );
    },
    max() {
      /* Currently vue-slider-component only supports limited precision. The backend is using
            ...
            found here: https://github.com/NightCatSama/vue-slider-component/issues/462#issue-634463578 */
      return parseFloat(
        this.sliderSettings.range.max[0].toFixed(MAX_DECIMAL_PRECISION),
      );
    },
    value() {
      return Array.isArray(this.valuePair)
        ? this.valuePair
        : this.valuePair[DATA_TYPE];
    },
    /**
    ...
    tooltips() {
      ...
      if (this.sliderSettings.tooltips) {
        return this.sliderSettings.tooltips.map((options) =>
          createTooltip(options ? "always" : "none"),
        );
      }
  ...
  methods: {
    ...
    validate() {
      ...
      if (typeof this.$refs.form.validate === "function") {
        ...
        errorMessage =
          validateEvent.errorMessage || "Current input is invalid.";
      }
...
<template>
  <div>
    <Label
      ...
      :text="label"
      :class="[
        `slider-${sliderSettings.orientation}-label`,
    ...
    >
      ...
      <ErrorMessage
        :error="errorMessage"
        :class="[
          `slider-${sliderSettings.orientation}-error`,
