...
export default {
  ...
  computed: {
    ...
    autoHeight() {
      return (
        !this.isSingleView && this.viewConfig.resizeMethod?.startsWith("view")
      );
    },
    classes() {
      let classes = [];
    ...
    origin() {
      // react to current browser environment
  ...
  mounted() {
    ...
    let getPublishedDataFunc =
      this.$store.getters["pagebuilder/interactivity/getPublishedData"];
  ...
  methods: {
    ...
    computeStyles(resourceBaseUrl) {
      ...
      let styles = this.nodeStylesheets.map((style) => {
      ...
      if (this.nodeConfig.customCSS?.length) {
        ...
        styles.push(
          `<style>${this.nodeConfig.customCSS.replace(
            /<(\/style)\b/gi,
            "\\00003c$1",
          )}</style>`,
        );
    ...
    computeScripts(resourceBaseUrl, accessToken) {
      ...
      let scripts = this.nodeJsLibs.map((lib) => {
    ...
    initIFrameResize() {
      ...
      if (conf.resizeMethod) {
        method =
          conf.resizeMethod.substring(prefix, prefix + 1).toLowerCase() +
        ...
        this.document.defaultView.postMessage(
          {
            ...
            viewRepresentation: JSON.stringify(
              this.nodeConfig.viewRepresentation,
            ),
    ...
    setValidationError(errorMessage) {
      return new Promise((resolve, reject) => {
        ...
        this.document.defaultView.postMessage(
          {
            nodeId: this.nodeId,
            namespace: this.nodeConfig.namespace,
            setValidationErrorMethodName:
              this.nodeConfig.setValidationErrorMethodName,
        ...
        this.cancelSetValidatorError = window.setTimeout(() => {
          let errorMessage =
            "Validation error message could not be set in the allocated time.";
          this.setLocalError(errorMessage);
          reject(new Error(errorMessage));
...
<template>
  <div
    :class="[
      ...
      { 'with-alert': displayAlert },
    ]"
  >
    <iframe :id="iframeId" ref="iframe" :class="classes" />
    <AlertLocal :active="displayAlert" @show-alert="showAlert" />
  </div>
</template>
