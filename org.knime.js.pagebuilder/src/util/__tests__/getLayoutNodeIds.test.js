...
describe("getLayoutNodeIds util", () => {
  let getPageFromRows = (rows) => ({
    wizardPageContent: { webNodePageConfiguration: { layout: { rows } } },
  });
  ...
  it("returns no nodeIds if the page is missing a valid layout", () => {
    let testCases = [
      ...
      getLayoutNodeIds({
        wizardPageContent: { webNodePageConfiguration: null },
      }),
      getLayoutNodeIds({ wizardPageContent: { webNodePageConfiguration: {} } }),
      getLayoutNodeIds({
        wizardPageContent: { webNodePageConfiguration: { layout: null } },
      }),
      getLayoutNodeIds({
        wizardPageContent: { webNodePageConfiguration: { layout: {} } },
      }),
      getLayoutNodeIds({
        wizardPageContent: {
          webNodePageConfiguration: { layout: { rows: null } },
        },
      }),
      getLayoutNodeIds({
        wizardPageContent: {
          webNodePageConfiguration: { layout: { rows: [] } },
        },
      }),
    ];
  ...
  it("returns nodeIds from content in a page layout", () => {
    let layoutNodeIds = getLayoutNodeIds(
      getPageFromRows([
        {
          columns: [
            {
              content: [{ nodeID: "1" }, { nodeID: "2" }],
            },
            {
              content: [{ type: "JSONLayoutHTMLContent" }],
            },
          ],
        },
        {
