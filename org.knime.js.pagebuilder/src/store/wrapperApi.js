...
export const actions = {
  ...
  async triggerReExecution({ dispatch }, { nodeId }) {
    ...
    if (!viewValues) {
      dispatch("handleError", {
        caller: "triggerReExecution",
        error:
          "Retrieving page values failed. Please check the page for errors.",
      });
      return;
    }
    dispatch("pollRPC", {
  ...
  updatePage({ dispatch }, { nodeId }) {
    ...
    dispatch("pollRPC", {
      ...
      config: {
        nodeId,
        rpcConfig: createJsonRpcRequest("ReexecutionService.getPage"),
      },
    });
  },
  ...
  setPage(
    { dispatch },
    { page, resetNodes = [], reexecutedNodes: reExecutedNodes = [] } = {},
  ) {
    consola.debug(
      "WrapperAPI store: setPage (page, resetNodes, reexecutedNodes)",
      page,
      resetNodes,
      reExecutedNodes,
    );
    try {
      if (page) {
        let nodeIds = reExecutedNodes?.length
          ? reExecutedNodes
  ...
  async singleRPC(_, { rpcConfig }) {
    ...
    if (typeof window.jsonrpc === "function") {
      ...
      try {
        ...
        // FIXME: My IDE says that "...Service.send" doesnt return any value? Is that a false positive?
        ({ result, error } = JSON.parse(
      ...
      error =
        "This functionality is not supported with the current browser. Please ensure the correct " +
