...
const addInteractivityId = (state, { id, subscriberOnly }) => {
  if (!state[id]) {
    state[id] = {
      // we don't need reactivity, so no need to use Vue.set()
      subscribers: [], // they will get notified when data changes
    };
  }
  if (!subscriberOnly && !state[id].data) {
    state[id].data = {
      // keeps the current data to be accessed on demand by getPublishedElement()
...
const getRelevantElements = (state, { id, filterIds, changedIds }) => {
  ...
  let relevantElements = data.elements.filter((value) =>
    filterIds.includes(value.id),
  );
...
const notifySubscribers = (state, { id, data, skipCallback, changedIds }) => {
  if (state[id]) {
    ...
    outdatedSubscriptions
      .reverse()
      .forEach((subInd) => state[id].subscribers.splice(subInd, 1));
  }
};
  ...
  data.elements.forEach((element) => {
    if (typeof element.id !== "undefined") {
      if (state[id] && state[id].data) {
        ...
        let matched = state[id].data.elements.filter(
          (existingElement) => element.id === existingElement.id,
        );
        // if element could not be found or the elements differ, add current id to list of changed ids
        if (
          !matched.length ||
          JSON.stringify(element) !== JSON.stringify(matched[0])
        ) {
...
const removeRows = (state, { id, rowsToRemove }) => {
  ...
  while (i--) {
    ...
    removedRows = removedRows.concat(
      rowsToRemove.filter((row) => curRows.includes(row)),
    );
...
const removePartialRows = (state, { id, rowsToRemove }) => {
  let curData = state[id].data;
  let filteredPartial = curData.partial.filter(
...
const assembleChangePayload = ({
  ...
  let modified = Boolean(
    removedRows.length +
      addedRows.length +
      partialRemovedRows.length +
      partialAddedRows.length,
  );
  if (modified) {
...
const processChangeset = (state, { id, data }) => {
  ...
  let partialAdded =
    data.changeSet.partialAdded && data.changeSet.partialAdded.length;
  ...
  let partialRemoved =
    data.changeSet.partialRemoved && data.changeSet.partialRemoved.length;
  let curData = state[id].data;

  // handle deleted rows
  let allRemovedRows = [];
  if (curData && curData.elements && removed) {
    allRemovedRows = removeRows(state, {
      id,
      rowsToRemove: data.changeSet.removed,
    });
  }
  let allRemovedPartial = [];
  if (curData && curData.partial && partialRemoved) {
    allRemovedPartial = removePartialRows(state, {
      id,
      rowsToRemove: data.changeSet.partialRemoved,
    });
  }
  ...
  let allAddedPartial = [];
  if (partialAdded) {
    allAddedPartial = addPartialRows(state, {
      id,
      rowsToAdd: data.changeSet.partialAdded,
    });
  }
...
const mapSelectionEventSource = function (
  changeSet,
  { addedRows, removedRows, mapping, curElementTarget },
) {
  ...
  addedRows.forEach((addedRow) => {
    if (mapping[addedRow]) {
      let addedNotExisting = mapping[addedRow].filter(
        (row) => !curRowsTarget.includes(row),
      );
  ...
  removedRows.forEach((removedRow) => {
    if (mapping[removedRow]) {
      let removedExisting = mapping[removedRow].filter((row) =>
        curRowsTarget.includes(row),
      );
...
const mapSelectionEventTarget = function (
  changeSet,
  ...
  for (let row in mapping) {
    let include = mapping[row].every((mappedRow) =>
      curRowsTarget.includes(mappedRow),
    );
    let partial = mapping[row].some((mappedRow) =>
      curRowsTarget.includes(mappedRow),
    );
    if (curElementTarget && curElementTarget.partial) {
      partial =
        partial ||
        mapping[row].some((mappedRow) =>
          curElementTarget.partial.includes(mappedRow),
        );
    }
    let includeAdded = mapping[row].some((mappedRow) =>
      addedRows.includes(mappedRow),
    );
    let includeRemoved = mapping[row].some((mappedRow) =>
      removedRows.includes(mappedRow),
    );
  ...
  changeSet.partialAdded = mappedPartial.filter(
    (row) => !curPartialRowsSource.includes(row),
  );
  changeSet.partialRemoved = curPartialRowsSource.filter(
    (row) => !mappedPartial.includes(row),
  );
};

// create a mapping for a selection event with a changeSet
const mapSelectionEvent = function (
  data,
  { mapping, sourceToTarget, curElementSource, curElementTarget },
) {
  if (!data || !data.changeSet) {
    throw new Error(
      "Selection event could not be mapped because no change set was present.",
    );
  }
  let mappedData = {
  ...
  if (
    mappedData.changeSet.partialAdded &&
    mappedData.changeSet.partialAdded.length < 1
  ) {
    delete mappedData.changeSet.partialAdded;
  }
  if (
    mappedData.changeSet.partialRemoved &&
    mappedData.changeSet.partialRemoved.length < 1
  ) {
 ...
 */
const handleSelectionTranslatorEvent = function (
  { dispatch, getters },
  { data, translator, targetId, sourceToTarget, skipCallback },
) {
  let mappedData = data;
  if (!translator.forward && translator.mapping) {
    let curElementSource = getters.getPublishedData(
      `selection-${translator.sourceID}`,
    );
  ...
  mappedData.mappedEvent = translator.sourceID;
  let id = sourceToTarget
    ? `selection-${targetId}`
    : `selection-${translator.sourceID}`;
...
const getTranslatorSourceCallback = function (
  { dispatch, getters },
  { translator },
) {
  let translatorCallback = (id, data) => {
    ...
    translator.targetIDs.forEach((targetId) => {
      handleSelectionTranslatorEvent(
        { dispatch, getters },
        {
          data,
          translator,
...
const getTranslatorTargetCallback = function (
  { dispatch, getters },
  { translator, targetId },
) {
  let translatorCallback = (id, data) => {
...
const subscribeSourceTranslator = function (
  { dispatch, getters },
  { translator },
) {
  dispatch("subscribe", {
    id: `selection-${translator.sourceID}`,
    callback: getTranslatorSourceCallback(
      { dispatch, getters },
      { translator },
    ),
    isTranslator: true,
...
const subscribeTargetTranslator = function (
  { dispatch, getters },
  { translator, targetId },
) {
  dispatch("subscribe", {
    id: `selection-${targetId}`,
    callback: getTranslatorTargetCallback(
      { dispatch, getters },
      { translator, targetId },
    ),
    isTranslator: true,
...
const updateSourceTranslator = function (
...
const updateTargetTranslator = function (
  { dispatch, commit, getters },
  { translator, targetId, clear },
) {
  commit("updateSubscriber", {
    id: `selection-${targetId}`,
    callback: getTranslatorTargetCallback(
...
export const mutations = {
  addSubscriber(state, { id, callback, elementFilter, isTranslator }) {
    addInteractivityId(state, { id, subscriberOnly: true });
    state[id].subscribers.push({
      callback,
      filterIds: elementFilter,
      isTranslator,
    });
  },
  updateSubscriber(
    state,
    { id, callback, elementFilter, isTranslator, clear },
  ) {
    if (state[id]) {
      if (clear) {
        // keep non-translated subscribers to maintain channel with non-updated elements
        state[id].subscribers = state[id].subscribers.filter(
          (subscriber) => !subscriber.isTranslator,
        );
        state[id].data = {
          elements: [],
        };
  ...
  removeSubscriber(state, { id, callback }) {
    if (state[id]) {
      let index = state[id].subscribers.findIndex(
...
export const actions = {
  subscribe({ commit, state }, { id, callback, elementFilter, isTranslator }) {
    ...
    if (state[id].data) {
      let relevantElements = getRelevantElements(state, {
        id,
        filterIds: elementFilter,
      });
      if (!relevantElements) {
  ...
  registerSelectionTranslator({ dispatch, getters }, { translator }) {
