...
describe("wrapper API store", () => {
  ...
  describe("rpc", () => {
    ...
    it("invokes a single RPC via action", async () => {
      ...
      const res2 = await store.dispatch("singleRPC", {
        nodeId: "foo",
        rpcConfig,
      });
      expect(sendMock).toHaveBeenCalledWith(
        "org.knime.js.cef.jsonrpc#1234",
    ...
    it("handles RPC polling with resolution after multiple call", async () => {
      ...
      let setPage = vi
        .fn()
        .mockReturnValueOnce({ shouldPoll: true })
      ...
      expect(singleRPC).toHaveBeenLastCalledWith(expect.anything(), {
        ...rpcConfig,
        rpcConfig: { ...rpcConfig.rpcConfig, id: expect.any(Number) },
      });
      expect(showAlert).not.toHaveBeenCalled();
  ...
  describe("getters", () => {
    it("returns the correct resource location for UI Extensions", () => {
      ...
      const path = "path";
      expect(
        store.getters.uiExtResourceLocation({
          resourceInfo: { baseUrl, path },
        }),
      ).toBe(baseUrl + path);
      ...
      expect(
        store.getters.uiExtResourceLocation({
          resourceInfo: { debugUrl, baseUrl, path },
        }),
      ).toBe(debugUrl);
    });
  });
